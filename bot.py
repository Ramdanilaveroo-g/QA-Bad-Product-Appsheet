...
# ════════════════════════════════════════════
#  GEMINI CLIENT (pakai google.genai yang baru)
# ════════════════════════════════════════════
...
# ════════════════════════════════════════════
#  HELPER: Baca label pakai Gemini 1.5 Flash
# ════════════════════════════════════════════
def read_label(image_bytes: bytes) -> dict:
    import re as _re

    prompt = (
        "Ini adalah foto label produk dari warehouse Astro Indonesia.\n"
        "Baca SEMUA teks yang ada di label putih dalam gambar ini.\n\n"
        "Label Astro selalu berisi format seperti ini:\n"
        "- Baris 1: nama produk (contoh: Tomat Merah Ekonomis Astro Farm 500gram)\n"
        "- ExpDate: tanggal expired (ABAIKAN)\n"
        "- VID: angka 4-6 digit (Vendor ID)\n"
        "- Baris angka panjang: itu SKU number (contoh: 216265;24052026 → SKU = 216265)\n"
        "- MFG: tanggal produksi\n\n"
        "PENTING untuk SKU:\n"
        "- Format di label: angka;tanggal (contoh: 216265;24052026)\n"
        "- Yang diambil hanya angka SEBELUM titik koma → 216265\n\n"
        "Return HANYA JSON ini, tidak ada teks lain:\n"
        "{\n"
        ' "sku_number": "angka sebelum titik koma / SKU number",\n'
        ' "product_name": "nama produk lengkap",\n'
        ' "mfg_date": "DD/MM/YYYY",\n'
        ' "vendor_id": "angka VID"\n'
        "}\n\n"
        "Konversi MFG date ke DD/MM/YYYY:\n"
        "- 16-May-2026 → 16/05/2026\n"
        "- 16/05/2026 → 16/05/2026\n"
        "Kalau tidak terbaca isi empty string."
    )
    ...
    try:
        ...
        log.info(f"Gemini result: {raw}")
        m = _re.search(r"\{[\s\S]*\}", raw)
        if m:
            data = __import__('json').loads(m.group())
            # Bersihkan SKU: ambil sebelum titik koma kalau ada
            sku = data.get("sku_number", "")
            if ";" in str(sku):
