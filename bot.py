...
# ════════════════════════════════════════════
def find_label_region(img: np.ndarray) -> np.ndarray:
    """Cari area label putih di foto, crop bagian teks (kanan label, skip QR code)."""
    h, w = img.shape[:2]
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)

    # Threshold untuk cari area putih (label)
    ...
    for cnt in contours:
        ...
        if area > (h * w * 0.05):  # min 5% of image
            x, y, cw, ch = cv2.boundingRect(cnt)
            ratio = cw / ch
            if 1.5 < ratio < 8:  # label biasanya landscape
                if area > best_area:
                    best_area = area
                    best = (x, y, cw, ch)
    ...
    if best:
        x, y, cw, ch = best
        # Ambil 55% kanan (area teks, skip QR code di kiri)
        text_x = x + int(cw * 0.35)
        text_w = int(cw * 0.65)
        cropped = img[y:y+ch, text_x:text_x+text_w]
        log.info(f"Label region found: {best}, text area: {text_x},{y}")
        return cropped

    # Fallback: ambil bagian tengah-kanan foto
    return img[:, w//3:]

def preprocess(image_bytes: bytes) -> np.ndarray:
    """Crop area teks label → grayscale → threshold untuk OCR."""
    ...
    # Crop area teks label (skip QR code)
    text_region = find_label_region(img)

    # Upscale 3x untuk akurasi lebih baik
    text_region = cv2.resize(text_region, None, fx=3, fy=3, interpolation=cv2.INTER_CUBIC)
    ...
    # Otsu threshold (otomatis pilih threshold terbaik)
...
def ocr_and_parse(image_bytes: bytes, sku_from_qr: str = "") -> dict:
    ...
    try:
        ...
        # 2. VID — format: "VID: 5051" atau "VID 5051"
        for line in lines:
            m = re.search(r"VID\s*[:\-]?\s*(\d{3,6})", line, re.IGNORECASE)
            if m:
                result["vendor_id"] = m.group(1)
                log.info(f"VID found: {m.group(1)}")
                break

        # 3. MFG Date — format: "MFG 16-May-2026"
        full_text = " ".join(lines)
        # Coba semua variasi format MFG
        for pattern in [
            r"MFG\s+(\d{1,2}[-/]\w{3,}[-/]\d{4})",
            r"MFG\s+(\d{1,2}[-/]\d{1,2}[-/]\d{4})",
            r"MFG[:\s]+(\d{1,2}\s+\w+\s+\d{4})",
        ]:
            m = re.search(pattern, full_text, re.IGNORECASE)
            if m:
                result["mfg_date"] = parse_date(m.group(1).strip())
                log.info(f"MFG found: {m.group(1)}")
                break

        # 4. Product name — baris pertama yang ada huruf & bukan keyword
        skip_kw = {"expdate","exp","vid","mfg","astro logo","barcode"}
        for line in lines:
            line_low = line.lower()
            # Skip baris yang pure angka atau keyword
            ...
            if len(line) > 8:
                ...
                if len(clean) > 8:
                    ...
                    log.info(f"Product found: {clean}")
                    break
...
def main():
    ...
    conv = ConversationHandler(
        ...
        states={
            ...
            # Fallback kalau ada foto di state lain yang tidak terduga
