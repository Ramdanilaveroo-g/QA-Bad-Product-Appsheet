...
================================================================
  ...
  Railway-ready | Gemini API (GRATIS) untuk OCR
================================================================
  Kolom Sheet "HUB":
...
GEMINI_API_KEY       = os.getenv("GEMINI_API_KEY", "")        # dari Google AI Studio
GOOGLE_CREDS_JSON    = os.getenv("GOOGLE_CREDENTIALS_JSON", "") # isi JSON credentials nya
...
# ════════════════════════════════════════════
#  USER STORAGE — Google Sheets tab "Users"
# ════════════════════════════════════════════
...
# ════════════════════════════════════════════
#  HELPER: OCR label pakai Gemini Vision (GRATIS)
# ════════════════════════════════════════════
def ocr_label(image_bytes: bytes, sku_from_qr: str = "") -> dict:
    hint = f'SKU sudah diketahui dari QR: "{sku_from_qr}". ' if sku_from_qr else ""
    prompt = f"""
Kamu sistem OCR warehouse QA Indonesia.
{hint}
Baca label produk ini dan return HANYA valid JSON tanpa teks lain:
{{
  "sku_number": "angka SKU lengkap",
  "product_name": "nama produk lengkap",
  "mfg_date": "format DD/MM/YYYY",
  "vendor_id": "angka VID 4-6 digit"
}}
- mfg_date: cari tulisan MFG, konversi ke DD/MM/YYYY
- vendor_id: cari tulisan VID
- Kalau tidak ada isi dengan ""
- Return JSON saja, TANPA teks tambahan apapun
"""
    try:
        img_part = {"mime_type": "image/jpeg", "data": base64.b64encode(image_bytes).decode()}
        resp     = gemini.generate_content([prompt, img_part])
        raw      = resp.text.strip()
        log.info(f"Gemini OCR: {raw}")
    ...
    return {"sku_number": sku_from_qr, "product_name": "", "mfg_date": "", "vendor_id": ""}
...
async def handle_label(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        sku_from_qr = scan_qr(img) or ""
        data        = ocr_label(img, sku_from_qr)
        if sku_from_qr:
            data["sku_number"] = sku_from_qr
        data["qty"] = DEFAULT_QTY
        ctx.user_data["data"] = data

        qr_info = "✅ QR berhasil discan" if sku_from_qr else "⚠️ QR tidak terbaca, pakai OCR"
        await wait.delete()
        await update.message.reply_text(
            f"📷 *Label dibaca!* _{qr_info}_\n\n"
