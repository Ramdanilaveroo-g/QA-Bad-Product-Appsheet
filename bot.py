...
# ════════════════════════════════════════════
#  HELPER: Baca label + QR pakai Gemini Vision
#  (Gemini bisa baca QR code dari foto langsung)
# ════════════════════════════════════════════
def read_label(image_bytes: bytes) -> dict:
    prompt = """
Kamu sistem OCR warehouse QA Indonesia.
Baca label/sticker produk ini dengan teliti.

Perhatikan:
1. Ada QR code atau barcode → baca isinya (biasanya angka panjang = SKU)
2. Cari teks: nama produk, MFG date, VID/Vendor ID

Return HANYA valid JSON tanpa teks lain:
{
  "sku_number": "isi QR code / barcode (angka panjang SKU)",
  "product_name": "nama produk lengkap",
  "mfg_date": "format DD/MM/YYYY",
  "vendor_id": "angka VID 4-6 digit"
}
- Kalau tidak ada isi dengan ""
- HANYA JSON, tanpa penjelasan apapun
"""
    try:
        ...
        log.info(f"Gemini label read: {raw}")
