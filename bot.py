...
# Coba berbagai kombinasi endpoint + model
GEMINI_ENDPOINTS = [
...
def call_gemini(image_bytes: bytes, prompt: str) -> str:
    """Coba semua endpoint Gemini sampai ada yang berhasil."""
    import base64
    b64 = base64.b64encode(image_bytes).decode()

    for version, model in GEMINI_ENDPOINTS:
        ...
        try:
            ...
            if resp.status_code == 200:
                log.info(f"✅ Berhasil: {version}/{model}")
                return resp.json()["candidates"][0]["content"]["parts"][0]["text"]
            else:
                log.warning(f"Gagal {version}/{model}: {resp.status_code}")
        except Exception as e:
            log.warning(f"Error {version}/{model}: {e}")

    raise Exception("Semua endpoint Gemini gagal! Pastikan GEMINI_API_KEY sudah diupdate di Railway Variables!")
