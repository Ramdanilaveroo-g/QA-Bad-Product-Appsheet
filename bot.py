...
# ════════════════════════════════════════════
#  HELPER: Baca label pakai Gemini 2.0 Flash
#  Lebih akurat, tetap GRATIS
# ════════════════════════════════════════════
def read_label(image_bytes: bytes) -> dict:
    import re as _re
    prompt = (
        "Kamu sistem OCR canggih warehouse QA Indonesia. "
        "Baca label produk Astro ini seperti Google Lens.\n\n"
        "Label berisi:\n"
        "- QR Code/Barcode = SKU number (angka panjang 10-15 digit)\n"
        "- Nama produk (contoh: Kangkung Konvensional 200gram)\n"
        "- MFG: tanggal produksi\n"
        "- VID: angka 4-6 digit (Vendor ID)\n\n"
        "Return HANYA JSON ini:\n"
        "{\n"
        ' "sku_number": "angka SKU dari QR atau teks",\n'
        ' "product_name": "nama produk lengkap",\n'
        ' "mfg_date": "DD/MM/YYYY",\n'
        ' "vendor_id": "angka VID 4-6 digit"\n'
        "}\n\n"
        "Konversi tanggal: 15-May-2026 jadi 15/05/2026. "
        "Kalau tidak ada isi empty string. HANYA JSON."
    )
    try:
        ...
        log.info(f"Gemini 2.0 Flash: {raw}")
